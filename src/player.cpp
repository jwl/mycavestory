...
void Player::lookDown() {
	...
	if (this->_grounded == true) {
		...
		this->playAnimation(
				this->_facing == RIGHT ? "LookBackwardsRight" : "LookBackwardsLeft");
	}
	else {
...
// Handles collisions with ALL slopes hte player is colliding with
void Player::handleSlopeCollisions(std::vector<Slope> &others) {
	for (int i = 0; i < others.size(); i++) {
		...
		// (b = y - mx)
		int b = (others.at(i).getP1().y -
				(others.at(i).getSlope() * fabs(others.at(i).getP1().x)));

		// Now get the player's center x
		int centerX = this->_boundingBox.getCenterX();

		// Now pass that X int othe equation y = mx + b (using our newly found b and x)
		// to get the new y position
		int newY = (others.at(i).getSlope() * centerX) + b - 8; // 8 is temporary fix
...
void Player::handleDoorCollision(std::vector<Door> &others, Level &level, Graphics &graphics) {
	...
	// This assumes the player is already colliding with the door object.
	for (int i = 0; i < others.size(); i++) {
